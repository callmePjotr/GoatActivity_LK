"""
Tracker Variante A - Verbessert
- Robuste Re-ID mit Gallery-Features und Spatial Priors
- Längere Track-Lebenszeiten für bessere Verdeckungs-Behandlung
- Erweiterte Feature-Speicherung für 2h Material
- Reduzierte ID-Fragmentierung

Benutzung: python tracker_variant_A_improved.py --yolo-json detections.json --video input.mp4 --out-json outA.json --out-csv outA.csv
"""
...
MAX_AGE = 120  # Erhöht von 60 auf 120 (4 Sekunden bei 30fps)
MIN_HITS = 3
TENTATIVE_MAX_AGE = 15
CONFIRMED_TO_LOST_THRESHOLD = 30  # NEU: Frames bis confirmed -> lost

REIDENTIFY_ENABLED = True
REIDENTIFY_WINDOW = 1800  # Erhöht auf 60 Sekunden
REIDENTIFY_APPEARANCE_THRESHOLD = 0.58  # Gelockert von 0.45
REIDENTIFY_SPATIAL_WEIGHT = 0.35  # NEU
...
class ImprovedTrackerA:
    def __init__(self, max_age=MAX_AGE, min_hits=MIN_HITS):
        ...
        self.frame_count = 0
        self.id_swaps = 0
        self.prev_active_tracks = {}  # track_id -> (cx, cy, color_feature)
        self.fps = 30  # falls fix, sonst später aus Video lesen

    def _detect_id_swaps(self, active_tracks):
        current = {}
        for t in active_tracks:
            ...
            track_id = t["track_id"]
            current[track_id] = (cx, cy)

        # Vergleiche mit vorherigem Frame
        for prev_id, (px, py, pfeat) in self.prev_active_tracks.items():
            if prev_id in current:
                continue  # normaler Track-Fortlauf

            # Suche neuen Track in Nähe
            for curr_id, (cx, cy) in current.items():
                dist = math.hypot(px - cx, py - cy)
                if dist < 50:  # Pixel-Schwelle (datasetabhängig)
                    self.id_swaps += 1
                    if DEBUG:
                        print(f"ID SWAP: {prev_id} -> {curr_id} @ frame {self.frame_count}")
                    break

        # Update Speicher
        self.prev_active_tracks = {
            t["track_id"]: (
...
def run_pipeline(yolo_json_path: str, video_path: Optional[str], out_json: str, out_csv: str):
    ...
    total_minutes = total_frames / tracker.fps / 60.0

    id_swaps_per_minute = tracker.id_swaps / max(total_minutes, 1e-6)

    print(f"ID Swaps total: {tracker.id_swaps}")
